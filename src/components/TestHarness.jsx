...
export default function TestHarness() {
    ...
    async function handleSendTurn() {
        if (!sessionId) { addLog('error', 'No active session. Initialize first.'); return }
        if (!message.trim()) { addLog('warning', 'Enter a counterparty message.'); return }
        setLoading(true)
        ...
        try {
            ...
            if (result.bluff_probability_updates?.length) addLog('info', `[BLUFF TRACKER] ${JSON.stringify(result.bluff_probability_updates, null, 2)}`)
            if (mode === 'coached') {
                setPendingResponse(result.natural_language_response)
                addLog('warning', `[COACHED MODE — PENDING APPROVAL]\n"${result.natural_language_response}"\n\nClick "Approve & Send" to dispatch.`)
            } else {
    ...
    async function handleEndSession() {
        ...
        try {
            ...
            setSessionId(null); setPendingResponse(null)
        } catch (err) { addLog('error', err.message) }
        finally { setLoading(false) }
    }
    ...
    const s = {
        ...
        log: { background: '#0f172a', border: '1px solid #1e293b', padding: '10px', height: '420px', overflowY: 'auto', fontFamily: 'monospace', fontSize: '12px' },
    ...
    return (
        <div style={s.container}>
            <h1 style={s.h1}>⚡ NEGOTIATOR-CORE — PHASE 1 TEST HARNESS</h1>
            <div style={s.status}>{sessionId ? `● ACTIVE SESSION: ${sessionId} | Mode: ${mode.toUpperCase()}` : '○ NO ACTIVE SESSION — Initialize to begin'}</div>
            <div style={s.section}><span style={s.label}>Domain:</span><input style={s.input} value={domain} onChange={e => setDomain(e.target.value)} disabled={!!sessionId} /></div>
            <div style={s.section}><span style={s.label}>BATNA Value:</span><input style={s.input} type="number" value={batnaValue} onChange={e => setBatnaValue(Number(e.target.value))} disabled={!!sessionId} /><input style={{ ...s.input, minWidth: '260px' }} placeholder="BATNA description" value={batnaDesc} onChange={e => setBatnaDesc(e.target.value)} disabled={!!sessionId} /></div>
            <div style={s.section}><span style={s.label}>Concession Budget:</span><input style={{ ...s.input, maxWidth: '80px' }} type="number" value={concessionBudget} onChange={e => setConcessionBudget(Number(e.target.value))} disabled={!!sessionId} /><span style={{ color: '#64748b' }}>%</span></div>
            <div style={s.section}><span style={s.label}>Mode:</span><select style={s.select} value={mode} onChange={e => setMode(e.target.value)}><option value="coached">Coached (human approves before send)</option><option value="autonomous">Autonomous (sends directly)</option></select></div>
            <div style={{ ...s.section, marginBottom: '16px' }}>
                {!sessionId ? <button style={s.btn} onClick={handleInitSession} disabled={loading}>{loading ? 'Initializing...' : 'Initialize Session'}</button> : <button style={s.btnDanger} onClick={handleEndSession} disabled={loading}>End Session</button>}
                <button style={s.btnGray} onClick={() => setLog([])}>Clear Log</button>
            </div>
            <div style={s.divider} />
            <div style={{ marginBottom: '8px' }}>
                <div style={{ color: '#94a3b8', marginBottom: '4px' }}>Counterparty Message:</div>
                <textarea style={s.textarea} value={message} onChange={e => setMessage(e.target.value)} placeholder="Type what the counterparty just said..." disabled={loading || !sessionId} onKeyDown={e => { if (e.key === 'Enter' && e.ctrlKey) handleSendTurn() }} />
            </div>
            <div style={{ ...s.section, marginBottom: '16px' }}>
                <button style={s.btn} onClick={handleSendTurn} disabled={loading || !sessionId || !message.trim()}>{loading ? 'Processing...' : 'Send Turn  (Ctrl+Enter)'}</button>
                {pendingResponse && <button style={s.btnSuccess} onClick={() => { addLog('success', `[COACHED RESPONSE APPROVED]\n"${pendingResponse}"`); setPendingResponse(null) }}>✓ Approve &amp; Send</button>}
            </div>
            <div style={s.divider} />
            <div style={{ color: '#64748b', marginBottom: '4px' }}>Turn Log:</div>
            <div style={s.log} ref={logRef}>
                {log.length === 0 && <div style={{ color: '#374151' }}>Log is empty. Initialize a session and send turns to see output.</div>}
                {log.map(entry => (
